...
# **`b1_flat_real.xml` is one fitted MuJoCo dynamics instance -- the domain the existing
# expert-policy/babble pipeline was validated in -- not a claim of real-world ground truth.**
# Corrected wording, 2026-09-13 (a prior edit here briefly called it "wrongly fitted" and swapped
# in `b1_flat.xml`; that was wrong and has been reverted). `b1_flat.xml` has generic/uniform
# placeholder motor dynamics and is documented, a few lines below, to fail open-loop CPG control
# outright (every sign/phase/frequency/duty combo tried, 8+ configs) -- it is validated only
# together with the specific trained PPO policy, not for babble. Use `b1_flat_real.xml` for
# anything without a trained policy in the loop, described precisely as "the MuJoCo domain this
# pipeline uses," never as "the true real B1."
