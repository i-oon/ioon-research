...
class RolloutBuffer:
    ...
    def __init__(self, buffer_size, state_shape, action_shape, device, mix=1,
        ...
        if critic_state_shape is not None:
            self.critic_states = torch.empty(
                (self.total_size, *critic_state_shape), dtype=torch.float, device=device)
            self.next_critic_states = torch.empty(
                (self.total_size, *critic_state_shape), dtype=torch.float, device=device)

    def append(self, state, action, reward, done, log_pi, next_state,
               critic_state=None, next_critic_state=None):
        self.states[self._p].copy_(torch.from_numpy(state))
        self.actions[self._p].copy_(torch.from_numpy(action))
        self.rewards[self._p] = float(reward)
        self.dones[self._p] = float(done)
        ...
        if self.critic_states is not None:
            if critic_state is None or next_critic_state is None:
                raise ValueError("critic_state and next_critic_state are required for asymmetric critic")
            self.critic_states[self._p].copy_(torch.from_numpy(critic_state))
            self.next_critic_states[self._p].copy_(torch.from_numpy(next_critic_state))
    ...
    def get(self):
        ...
        if self.critic_states is not None:
            result += (self.critic_states[idxes], self.next_critic_states[idxes])
        return result
