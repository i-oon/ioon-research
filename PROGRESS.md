...
**adversarial ทำงานตามที่ออกแบบ แต่ไม่ช่วย** (`m3d_adv01` เทียบ `m3d_bracketed` เฉลี่ย 7 epochs)

| | control | adversarial |
|---|---|---|
| heldout | **0.101** | 0.124 (แย่ลง 1.23×) |
| z-gap | 27.2× | 5.9× |
| **x-gap** | 11.1× | **19.1×** |

decoder ย้ายจาก `z` ไปพึ่งภาพจริง (x-gap ขึ้น 1.7 เท่า) แต่ transfer แย่ลง
และ probe ไต่กลับ 0.337 → 0.457 หลัง warmup เต็ม แปลว่า ITM ชนะ adversary ในระยะยาว
