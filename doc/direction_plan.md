...
| 2i | **Training window** | is one timestep the right pair to train the forward model on? F87 measured the forward model barely reading the latent (frame dominates 28x, a latent from another behaviour costs 0.25%), and the gradient balance ruled out the weighting as the cause. The task being too easy is what was left | **no**, and widening it is a **paired** change. `cfg.frame_stride` widens the ITM's two frames; `cfg.action_chunk` (default 0 = follow `frame_stride`) widens the command target to match. **Measured at stride 5 and 10 with only the frames widened (F88): z-usage rose 1.6x -- sweep z 4.257 -> 6.764 -- and val `motion` collapsed 0.218 -> 0.928, the level of predicting the training mean**, because `z` was summarising k steps and being graded on one. LAC-WM chunks both sides. Chunking implemented and unit-tested 2026-08-24; chunk 1 is bit-identical, so every recorded run and checkpoint is unaffected | **retrain stride 5 and 10 with chunking, then `ftm_uses_z.py` *and* `screen_behaviour_channels.py` on each** -- the gain has to survive with the decoder intact, and F54's warning that stride-scale pairs lose across robots is still unresolved |
