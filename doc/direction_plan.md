...
| 2k'' | **Collect the matched set** | **done** — `data/beh12_hex_flat` and `data/beh12_b1_flat`, 48 clips each. Hexapod `--spin` 0.05 / 0.15 / 0.29 / 0.56 against B1 `--wz` 0 / 0.055 / 0.139 / 0.294, matched within 3% on dimensionless turn rate with Froude held at 0.12-0.13 on both sides. Speed matched by widening the hexapod's foot path rather than moving either robot's pace (F71, F72) | done |
...
| 2i | **Training window** | is one timestep the right pair to train the forward model on? **Now the highest-value open step**: F87 measured the forward model barely reading the latent (frame dominates 28x, a latent from another behaviour costs 0.25%), and the gradient balance rules out the weighting as the cause — recon is 79% of the loss and 5-13% of the gradient into `z`. The task being too easy is what is left, and this is the fix for it | **no** — at 20 Hz, `t → t+1` is 50 ms, a nineteenth of a 0.95 s stride. In-domain, stride-scale pairs roll better at every multi-step horizon (F54). **Implemented 2026-08-24**: `cfg.frame_stride` sets how far apart the ITM's two frames sit, threaded through all three dataset classes. Smoke-tested at stride 10 -- val recon rises 2.99 to 3.25, which is the harder task the change exists to create. **Run stride 5 and 10, and measure both `ftm_uses_z.py` and `screen_behaviour_channels.py` after each**: F54 found the long baseline winning every horizon in-domain and *losing* every horizon across robots, so a gain in z-usage bought with a loss in transfer is a trade to report rather than a fix | **ready to run** |
