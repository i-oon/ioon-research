...
(FINDINGS F10) → ตัวเลข held-out จาก run เดียวตีความไม่ได้ ต้องมี error bar
...
ผลทั้งหมดพร้อมตัวเลขและคำสั่ง reproduce อยู่ใน **[FINDINGS.md](FINDINGS.md)** F13–F16 ส่วนนี้บันทึกลำดับเหตุการณ์
...
| สมมติฐาน | สถานะ |
|---|---|
| ข้อมูลไม่อยู่ในภาพ | ตัดออก (F17) |
| `z` มีร่างกายให้ lookup | ตัดออก (F18 เอาออกแล้วไม่ช่วย) |
| น้ำหนัก loss ผิด | ตัดออก (F9) |
| decoder ใหญ่เกินไป | ตัดออก (F4 ลดแล้วแย่ลง) |
...
| ลดขนาด decoder head (F4) | แย่ลง 1.4–2.1× |
| ตัดโค้ดร่างกายออกจาก `z` (F18) | ใช้ภาพมากขึ้น 2× แต่ transfer แย่ลง 1.21× |
| **ให้ทางเข้าถึงภาพตรง ๆ (F19)** | **ใช้ภาพน้อยลง 7.6× transfer เท่าเดิม** |
| **เพิ่มจำนวนหุ่น (F14, F15)** | **ดีขึ้น 3.1× — ทางเดียวที่ได้ผล** |
...
- FTM ไม่เปลี่ยน (`z` ช่วย 1.03× เทียบ control 1.07×) — **F20 ยังยืน** `L_recon` ยังไม่ได้หล่อหลอม `z`
...
`z` ช่วยได้อย่างมาก 23% ของ `L_recon` วัดจริงได้ 3–7% (F20) — **`z` ไม่ได้ไร้ประโยชน์ มันถูกกลบ**

รวมกับ F20: 99% ของ gradient ไปที่ term ที่เป้าหมายเป็น noise ล้วน เหลือ 1% ให้ `L_motion`
...
→ บันทึกเป็น F24 และใส่ขอบเขตให้ F21 ว่าใช้ได้เฉพาะร่างในกรอบที่ข้อมูลครอบคลุม
...
|---|---|
| F20 FTM ไม่ต้องการ `z` (1.03x) | ไม่มีอะไรที่ `z` ต้องส่งให้ |
| F16 decoder ทำ lookup | `e_t` ตอบได้อยู่แล้ว `z` เหลือหน้าที่แค่บอกว่าร่างไหน |
| F22 `z` เป็นเฟสการเดิน 83-89% | เฟสคือสิ่งที่สองเฟรมมีร่วมกัน |
| F23 ท่าดีขึ้นแต่ระยะทางไม่ดีขึ้น | ไม่เคยถูกถามเรื่องการเคลื่อนที่ |
...
แก้ FINDINGS F20 แล้ว และยกเลิกรัน `lag1_noaug`
...
**ในไปป์ไลน์เดิม forward model ไม่ได้มีส่วนช่วยอะไร** ซึ่งตามมาจาก F25 โดยตรง

ของแถมที่ไม่คาด: ตัดมันทิ้งแล้ว decoder **ย้ายไปพึ่ง `z` มากขึ้นและพึ่งภาพน้อยลง**
แปลว่า `L_recon` เคยผลักให้ decoder ไปอ่านภาพ ซึ่งเป็นทิศที่ F16 ต้องการ
แต่มันทำโดยกิน gradient 99% แล้วไม่ได้ความแม่นยำกลับมาเลย

ต้องถามใหม่ภายใต้ `action_lag 1` เพราะตอนนี้ `z` มีงานทำแล้ว → บันทึกเป็น F46
...
**ชนะทุกระยะ FTM เรียนพลวัตได้จริง** → F46 และปรับขอบเขต F46 กับ F26

F20/F46 ยังถูก — `L_recon` ไม่ช่วยการถอด action แต่ "ไม่ช่วยการถอด action" กับ "ไม่ได้เรียนอะไร"
...
→ F20 กับ F46 ของเราเป็นข้อมูลใหม่ ไม่ใช่การขัดแย้ง และการวัดของเราแข็งกว่าเขาในจุดนั้น

→ บันทึกเป็น F46, Q10, Q11
...
F20 ที่เคยสรุปว่า "ไม่มีการปรับความแรง augmentation แบบไหนที่กู้สัญญาณได้" ถูกแต่ไม่ครบ
เพราะหมุนได้แค่ปุ่มเดียว → บันทึกเป็น F27
...
ความผิดของโมเดลมีรูปร่างเดียวกับช่องว่างในข้อมูล → F28
...
→ เสนอเป็นขั้นตอนตรวจความพร้อมของเดตาเซ็ตก่อนเทรน บันทึกเป็น F29
...
F22 วัดสัดส่วนของ latent บนห้าร่างที่เทรน เพราะการแยก variance ต้องมีทุกร่างครบทุก timestep
...
**เป็นขอบเขตเดียวกับ F24 กับ F28 มาจากทิศทางที่สาม**
...
→ บันทึกเป็น F30 ปรับขอบเขต F22 และแก้สไลด์ 5 กับ 12
...
ใส่กฎลง `sim/make_leg_morphology.py` แล้ว จะปฏิเสธพร้อมบอกว่าเกินกี่มิลลิเมตร → F31
...
รันนี้แสดงว่าภาพนั้นอยู่ร่วมกับ latent ที่เป็นรหัส embodiment 33% ได้สบาย → F32
...
**คำถามเปลี่ยนรูป** จาก "forward model ที่แช่แข็งย้ายหุ่นได้ไหม" (F44 วัดแล้ว: ไม่ได้ 0.57–0.71x
...
**ผล (F45)** ITM+FTM จาก `stage1_m3d_cross` ปรับบนคลิป B1 เทียบกับสถาปัตยกรรมเดียวกันที่สุ่มใหม่
...
### 18.38 วัด F26 ใหม่ — ข้อสรุปเดิม แต่เหตุผลในสไลด์ผิด (2026-08-16)

ตาราง horizon ใน slide 11 ยึดเลขจาก F26 ซึ่ง fit บนสี่คลิป = 264 ตัวอย่าง ต่อฟีเจอร์ 1,408 ตัว
`fwd_m3d` มี 26 คลิปของ `c10f10t10` จึง fit ได้ 18
...
แก้ทั้งสี่จุดแล้ว (slide 1, slide 18 สองแห่ง, FINDINGS F32) พร้อมหมายเหตุห้ามเขียนแบบเดิมอีก
...
F45 บอกว่า pretrained ชนะ scratch แต่ตีความได้สองแบบ และตารางนั้นแยกไม่ออก เพราะทั้งสอง
...
และ `scratch` ซ้ำรอย F45 ได้ (0.98/0.97/0.91/0.85 เทียบ 0.98/0.96/0.89/0.82) คนละ run คนละ checkpoint
...
ตัวเลขทั้งหมดอยู่ใน **[FINDINGS.md](FINDINGS.md)** F50–F53 ส่วนนี้บันทึกลำดับเหตุการณ์และสิ่งที่พลาด

### จุดเริ่ม: การวัดที่ตอบไม่ได้

F49 บอกว่าระดับลำตัวคือระดับเดียวที่หุ่นสองตัวทับกัน (Froude 0.155 เทียบ 0.159) จึงลองวัดว่า `z`
...
`_archive/ik_walk_speed5` 5 ความเร็ว 67 คลิป Froude 0.113–0.221 ตรงกับ B1 ที่ 0.121–0.216
...
ท่าปลายมือร่วม — แต่ locomotion ไม่มีปริมาณแบบนั้น ต้องหาเอง และ F49 บอกว่าระดับขาใช้ไม่ได้
...
`fwd_hex7speed` = 5 ความเร็วคงที่ + ramp ขึ้น + ramp ลง = 91 คลิป
...
แล้วเอามารันบนเฟรมชุดเดียวกัน วัด correlation ของผลทำนาย (F59)
...
อ่าน `doc/LATENT ACTION ROBOT FOUNDATION WORLD MODELS FOR CROSS-EMBODIMENT ADAPTATION.pdf` จริง (F60)
...
ให้ที่ R² 0.676 → F57 คือเงื่อนไขที่เปเปอร์เขาได้ฟรีจนไม่ต้องเขียนถึง
...
F61 บอกว่าช่องอื่นนอกจากความเร็วเดินหน้าไม่ผ่านเกต **เพราะมันเป็นค่าคงที่ในข้อมูลของเรา** —
...
`--no_rephase`) — เหตุผลอยู่ใน F62 ตารางนั้นคือบันทึกเดียวที่เหลือ

**การปรับ gain/offset ต่อขา (F64)** ปิดช่องเรขาคณิตได้จริง (0.056 → 0.0001) แต่ท่าเดิน **แย่ลงทุกช่อง**
เพราะคำนวณในเฟรมที่โคลงตามตัวเอง — ผลลบที่เก็บไว้เพราะเหตุผลใช้ได้ทั่วไป

### ส่วนที่ 2 — บั๊กสี่ตัว ทั้งหมดอยู่ในโค้ดเดิม

```
F65  เฟรมเรตไม่ตรง   B1 เรนเดอร์ 50 Hz แมลง 20 Hz
                     หนึ่ง transition = 20 ms ฝั่งหนึ่ง 50 ms อีกฝั่ง
F66  หมุนคนละทาง     จับคู่ที่ |ŵ| ซึ่งไม่มีเครื่องหมาย ตรงกัน 5% ทั้งที่หมุนสวนกัน
F69  ตัวคุมทิศ B1    เป็น P ล้วน จึงเหลือ drift ค้าง +2°/s ในทุกคลิป
F70  วัดในกรอบโลก    "ความเร็วเดินหน้า" = ความเร็วจริง × ยังหันตามแกน x โลกแค่ไหน
```

**ทั้งสี่ตัวมองไม่เห็นจากค่าสรุป และทุกตัวโผล่ตอนเอาไปเทียบกับของจริงเท่านั้น**

- F65 โผล่ตอนคำนวณ Froude ของ B1 แล้วได้ 0.054 ทั้งที่ calibrate ไว้ 0.128
- F66 โผล่ตอนดู yaw **แบบมีเครื่องหมาย** แทนค่าสัมบูรณ์
- F69 โผล่ตอนดูสัญญาณ **ภายในคลิป** แทนค่าเฉลี่ย — `ki=5.0` ให้ค่านิ่ง 0.0732 เทียบเป้า 0.0736
  (ตรง 0.5%) แต่ข้างในคลิปแกว่ง 0.19 → 0.017 → 0.024 → 0.15 คือ ringing ล้วน ๆ
- F70 โผล่ตอนผู้ใช้ไม่เชื่อคำอธิบายเชิงรูปร่างที่ผมให้ไว้
...
1. **สรุปว่า "ให้ความหลากหลายแล้วไม่ได้ผล" จากการวัดด้วย frozen encoder** — ซึ่งคือสภาพ *ก่อนสอน*
   ...
   ช่องที่ใช้ได้จริงก็จะถูกตัดทิ้งด้วย → ถอนข้อสรุปแล้ว (F68)
...
2  body head forward    วัดซ้ำ F59 บนข้อมูลที่แก้เฟรมเรตแล้ว (F65)
...
F77 สรุปว่าทางแก้คือ **ทำให้งานทำนายยากขึ้น** ไม่ใช่ปรับน้ำหนัก loss เพราะ recon กินกราเดียนต์
...
ประโยคที่ F77 ยกมาอ้างเรื่องเฟรม พูดถึง action อยู่ในประโยคเดียวกัน
...
ถ้าได้อย่างเดียวคือยังไม่ได้อะไร และต้องวัด transfer ด้วย เพราะ F47 วัดไว้ว่าคู่เฟรมห่างชนะในบ้าน
...
F83 วัดว่า action ของ B1 เล่นซ้ำแล้วล้ม 0 จาก 8 แล้วถูกอ่านต่อว่า "ลูปฟิสิกส์บน B1 ไม่มีทางทำได้"
**ตาราง F83 บอกแคบกว่านั้น** — รอด 289, 154, 72, 58 ก้าวที่ 50 Hz คือ **หกวินาที** ที่คำสั่งช้าสุด
...
F87 แก้, F88-F91 ใหม่, Q15 ใน OPEN_QUESTION (คำตอบเรื่อง Diffusion ที่เคยอยู่แต่ในสไลด์)
F89 เก็บตารางหลักฐานจากสไลด์ที่ตัดออก ตรวจแล้วตัวเลขจากสไลด์ที่ตัด **166 จาก 166 ตัว**อยู่ในเอกสาร
...
ลงไปอยู่ข้างล่าง **dose-response ของการเลี้ยวใน F97 กำลังวัดคลิปที่ใช้สตาร์ทหุ่น ไม่ใช่เป้าหมาย** (F100)
...
การเดินตรง** ตัวเลข 15/15 ของ F85 เหลือ 14/15 และ error กลางเกือบเท่าตัว — ผลไม่ล้ม แต่ระยะห่างหาย
...
การเลื่อนของลำตัวไปข้างหน้าเป็นสัญญาณที่ใหญ่ที่สุดในภาพ** ซึ่งตรงกับที่ F97 สรุปจากฝั่งข้ามร่าง

**และการทดสอบสุดท้ายของวันคือความเร็ว ซึ่งเป็นผลลบ** Froude เป็นตัวเลขไร้หน่วย ออกแบบมาให้เทียบข้าม
ขนาดตัวได้ (F49) เพราะฉะนั้นความเร็วข้ามร่างวัดได้จริง ไม่ใช่ `n/a` อย่างที่เขียนไว้ตอนแรก **เจ็ดเป้าหมาย
เดินหน้าครอบช่วง Froude 0.129-0.222 คือ 1.72 เท่า**

**corr(เป้าหมาย, ที่ B1 ทำได้) = +0.074** และที่สำคัญกว่าคือ **corr(เป้าหมาย, vx ที่แพลนเนอร์เลือก)
= −0.167** — มันไม่ได้เลือกท่าที่เร็วขึ้นให้เป้าหมายที่เร็วขึ้นเลย เลือก vx เฉลี่ย 0.354-0.375 เหมือนกัน
ทุกเป้าหมาย **และคลังก็ไม่ใช่ข้อจำกัด** ผู้สมัครของ B1 ครอบช่วง Froude 0.126-0.206 พอดี

**แต่ลูปเดียวกันควบคุมความเร็วได้เมื่อเป้าหมายเป็นหุ่นตัวเดียวกัน** `hex_unseen_commit3` ผ่านเกณฑ์ 50% error
กลาง 14.8% **การควบคุมแบบมีระดับมีอยู่ภายในร่าง และหายไปเมื่อข้ามร่าง** บนปริมาณที่สร้างมาเพื่อให้
เทียบข้ามร่างได้พอดี

**เพราะฉะนั้นข้ออ้างที่ป้องกันได้แคบกว่าที่คุยกันทั้งวัน — สิ่งที่ข้ามร่างคือ *ประเภท* ของการเคลื่อนไหว
ไม่ใช่ *ปริมาณ*** และมันตอบคำถาม "เดินหน้าง่ายเกินไปหรือเปล่า" ไปในทางที่ยอมรับว่าใช่ สิ่งที่ข้ามได้คือ
หมวดหมู่ ซึ่งเป็นสิ่งที่การเลื่อนของลำตัวในภาพพาข้ามไปได้ (F102)

**และคำถามสุดท้ายของวันมาจากผู้ใช้ที่ไม่เชื่อประโยคที่ผมเสนอ** — *"มันจะมีความหมายก็ต่อเมื่อเป็น
zero-shot ไม่ต้อง fine-tune ฉันว่ามันแชร์ได้เพราะ fine-tune เป็นหลัก"* ทดสอบด้วยบันไดสามขั้น
ตั้งค่าเหมือนกันหมด เป้าหมายเดียวกัน

| | เดินหน้า `speed_c5.8` | เลี้ยว `turn_s0.56` |
|---|---|---|
| world model แช่แข็ง ฟิตแค่ projector | **5% ± 0** | 2% ± 2 |
| ปรับ ITM+FDM แยก MSE | **28% ± 5** | 12% ± 2 |
| ปรับร่วม MSE | **32% ± 7** | 2% ± 2 |
| ปรับร่วม + InfoNCE | **74% ± 3** | 32% ± 5 |
| สุ่ม | 33% | 33% |

**ขั้นแรกต่ำกว่าสุ่ม ไม่ใช่แค่อ่อน** latent ที่ pretrain จากแมลงใช้กับหุ่นสี่ขาไม่ได้เลย และ **ขั้นที่สอง
อยู่ที่เส้นสุ่มพอดี** คือปรับแบบที่ต้นฉบับทำ (MSE ทั้งสามขั้น) แล้วไม่ได้อะไรขึ้นมา

**สิ่งที่ทำให้ข้ามร่างได้คือ contrastive term ในขั้น adapt** ซึ่งเป็นกลไกเดียวกับ F88 ที่เคยวัดบนคลิป
ที่บันทึกไว้ (19% → 50%) **ตอนนี้มันตัดสินลูปฟิสิกส์จริงด้วย (32% → 71%)** เป็นการยืนยันที่แรงกว่าตัว
F88 เอง

**เพราะฉะนั้นข้ออ้างข้ามร่างเป็นเรื่องของ objective ตอน adapt ไม่ใช่เรื่องของตัวแทนที่ pretrain มา**
และประโยค "โมเดลไม่เคยเห็นหุ่นสี่ขาตอน pretrain" ต้องเลิกใช้ เพราะจริงตามตัวอักษรแต่ทำให้เข้าใจผิด
ว่า latent ข้ามร่างได้เอง (F103)

**และตัวควบคุมที่ขาดไปก็รันเสร็จในคืนเดียวกัน** rung2 กับ rung3 ต่างกันสองอย่าง — ปรับแยกกับปรับร่วม
และ MSE กับ InfoNCE — เพราะฉะนั้นยังสรุปไม่ได้ว่า contrastive คือคนทำงาน `stage3_b1_full.pt` คือ
การปรับร่วมด้วย MSE ล้วน มีอยู่บนดิสก์แล้ว

| | ep1 เดินหน้า | ep1300 เลี้ยว | ep2301 เดินข้าง |
|---|---|---|---|
| WM แช่แข็ง ฟิตแค่ projector | 5% | 5% | 38% |
| ปรับแยก MSE | 32% | 9% | 29% |
| **ปรับร่วม MSE** | **38%** | 0% | 25% |
| ปรับร่วม + InfoNCE | **71%** | 37% | 0% |
| สุ่ม | 33% | 33% | 17% |

**การปรับร่วมด้วย MSE ได้ 38% ซึ่งเท่าเส้นสุ่ม** เพราะฉะนั้นสิ่งที่ทำให้ข้ามร่างได้คือ loss ไม่ใช่ขั้นตอน
ไม่ใช่ข้อมูล ไม่ใช่สถาปัตยกรรม **และไม่ใช่งบเทรนด้วย เพราะฝั่ง MSE รัน 15000 ก้าวเทียบกับ 12000
ของฝั่ง InfoNCE คือได้มากกว่า 25% แล้วยังแพ้** (F103)

**และวัดความกว้างปิดท้าย** MuJoCo ทำซ้ำได้บิตต่อบิต (F95) เพราะฉะนั้นความกระจายต้องมาจากคลิป
เป้าหมาย ไม่ใช่จากการรันซ้ำ — สี่คลิปต่อเงื่อนไข บนสองอาร์มที่ต่างกันแค่ loss

**การเดินหน้าไม่ทับกันเลย** รันที่ดีที่สุดของ MSE คือ 38% รันที่แย่ที่สุดของ InfoNCE คือ 71%
**และการเลี้ยวไม่ผ่านเส้นสุ่มทั้งสองอาร์ม** — 37% ที่เคยดูเหมือนผลลัพธ์ คือขอบบนของช่วง 28-37
รอบเส้นสุ่ม 33 ส่วน MSE ได้ 2% ซึ่งต่ำกว่าสุ่มมาก คือหลีกเลี่ยงท่าเลี้ยวอย่างเป็นระบบ (F103)

**และงานสุดท้ายของวันคือการจัดระเบียบ ซึ่งพาไปเจอข้อบกพร่องที่ใหญ่ที่สุด** ผู้ใช้เปิดดู
`results/dataset/preview_beh12/` แล้วทักว่า B1 ถูกอัดมาครึ่งตัว วัดแล้วพบว่าเป็นเรื่องจริงและ
กว้างกว่าที่ทัก

```
                     พื้นที่ตัวที่เห็นน้อยสุด   เฟรมที่ชนขอบภาพ   กล่องล้อมตัว
B1  side_R_lvl1            42%              100%           157 x 96 px
B1  เดินหน้า/เลี้ยว          69-76%           36-47%
hexapod  ทุกเงื่อนไข         83-94%             0%           118 x 66 px
```

**หุ่นแมลงไม่เคยชนขอบภาพเลยทั้ง 48 คลิป ส่วน B1 ชนใน 36-100% ของเฟรม** และเกิดในลูปด้วย —
B1 เหลือ 43-66% ขณะที่แมลงอยู่ที่ 89-93% **B1 ยังใหญ่กว่าในภาพราว 33% ทุกมิติ** ซึ่งเป็นความ
ไม่สมมาตรที่ไม่เคยมีใครวัด

**แล้วผู้ใช้ชี้ต่อว่าพื้นก็เป็นปัญหา** วัดรอบแรกได้ว่าพื้นหลังของ B1 แปรปรวนกว่า 48% (sd 5.53 เทียบ
3.74) และผมสรุปว่าเป็นลายพื้น **ผู้ใช้ทักว่าพื้นฝั่งแมลงดูเรียบดีอยู่แล้ว จึงแยกส่วนวัดใหม่**

| | รวม | แสงไล่เฉด | ลายพื้นความถี่สูง |
|---|---|---|---|
| B1 | 5.53 | **5.30** | 0.99 |
| hexapod | 3.74 | **3.37** | 1.02 |

**พื้นเรียบทั้งสองฉากและเรียบเท่ากัน** ความต่างเกือบทั้งหมดคือการไล่เฉดของแสง **ข้อสรุปสามข้อที่ผม
เขียนจาก sd ดิบจึงถูกถอน** — ที่ว่า encoder อ่านตำแหน่งจากลายพื้นได้ ที่ว่ามันแยกหุ่นสองตัวได้จากพื้น
และที่ว่าต้องแก้พื้นก่อนถอยกล้อง **สิ่งที่เหลือคือกรอบภาพกับขนาดปรากฏ**

**ยังไม่ใช่สาเหตุที่พิสูจน์แล้วว่าเดินข้างล้ม** เพราะการเดินหน้าก็ถูกตัดขอบพอ ๆ กันแล้วยังได้ 74%
**แต่มันทำให้ทุกตัวเลขฝั่ง B1 กลายเป็นผลชั่วคราวจนกว่าจะเรนเดอร์ใหม่** (F104)

**แก้แล้วในคืนเดียวกัน** `--cam_fov 25` ทำให้หุ่นอยู่ในเฟรมครบ และระหว่างตรวจก็เจอข้อบกพร่องที่สอง
ซึ่งไม่เกี่ยวกับมุมกล้องเลย — **กล้องฝั่ง B1 ไม่เคยถูกตรึงที่จุดเดียว** ทุกคลิปจึงมีพื้นหลังของตัวเอง
ขณะที่ฝั่งแมลงเหมือนกันเป๊ะทั้ง 48 คลิป `--spawn` มีอยู่ในโค้ดเพื่อแก้เรื่องนี้แต่ไม่เคยถูกใช้

**และการเรนเดอร์รอบแรกถูกตีกลับด้วยตา** เพราะมุมกว้างขึ้นไปถึงขอบพื้น 15 m ทำให้เกิดแถบตรงพาด
หนึ่งในสามของภาพ **เกณฑ์สองข้อที่เขียนไว้ล่วงหน้าผ่านทั้งคู่** — ตัดขอบ 0% พื้นหลังต่างกัน 0.22 —
...
| sd พื้นหลัง | 5.48 | **3.67** | 3.80 |
| ขอบพื้นหลังที่แรงสุด | 6.34 | **3.50** | 4.32 |
| พื้นหลังต่างกันระหว่างคลิป | 2.79 | **0.23** | 0.14 |
| bbox | 136 px | 99 px | 115 px |
| ความต่างของหุ่นจากพื้น | 24.7 | 25.5 | 39.8 |

**การเลือกมุมต้องกวาดบนครบ 48 คลิป ไม่ใช่คลิปตัวแทนเงื่อนไขละหนึ่ง** ที่ 23° ตัวแทนทั้งสิบสอง
ผ่าน 0% แต่ทั้งชุดได้ 1% เพราะมีสองคลิปใน `side_L_lvl1` ที่เดินไกลกว่าตัวแทนของมัน **ความแปรปรวน
ภายในเงื่อนไขเดียวกันมองไม่เห็นจากการตรวจแบบตัวแทน** 24° ผ่านครบทั้ง 48

**สิ่งที่ยังต่างจากแมลงคือสี ไม่ใช่กรอบภาพ** B1 เทาบนพื้นเทาได้ค่าความต่าง 25.5 ส่วนแมลงสีส้มได้
39.8 **ช่องว่างนี้มีมาก่อนแล้ว** ของเดิมอยู่ที่ 24.7 และการถอยกล้องทำให้ดีขึ้นเล็กน้อย การแก้ต้อง
เปลี่ยนสีหุ่น ซึ่งจะปนกับการแก้กรอบภาพในการฟิตรอบเดียวกัน จึงเว้นไว้และบันทึกไว้แทน

**ความแปรปรวนของพื้นหลังเป็นสัญญาณรบกวน ไม่ใช่ทางลัด** — สองคลิปของเงื่อนไขเดียวกันต่างกัน 4.03
พอ ๆ กับข้ามเงื่อนไข 4.43 จึงไม่เคยรั่วคำตอบ แต่ก็ควรเอาออกอยู่ดี **และมีตัวเลขหนึ่งที่แย่ลง** พื้นหลัง
...
ไม่ได้รันใหม่เลยเพราะ state เก็บอยู่ในไฟล์แล้ว **เหลือฟิต stage 3 ใหม่ ซึ่งต้องใช้ fibo7** (F104)

**และคำถามสุดท้ายของคืนก็มาจากการเปิดวิดีโอดูอีกครั้ง** — *"รู้ตัวไหมว่าอันนี้ไม่ได้เลี้ยว"* ชี้ไปที่
`b1_turn_wz0.00` ซึ่ง `wz = 0.00` คือสั่งอัตราหมุนเป็นศูนย์ **วัดแล้วไม่ใช่แค่เลี้ยวน้อย แต่เป็นคลิป
เดินตรงตัวเดียวกันเป๊ะ** forward Fr +0.1259 กับ yaw +0.0008 เท่ากับ `speed_vx0.30` ทุกหลัก
**ชุดสิบสองเงื่อนไขมีพฤติกรรมจริงสิบเอ็ดแบบ**

**มันเอียงสองตระกูลไปคนละทาง** เลือก `wz0.00` เป็นการทำ*ถูก*สำหรับเป้าหมายเดินหน้าแต่ถูกนับว่าผิด
และเป็นการทำ*ผิด*สำหรับเป้าหมายเลี้ยวแต่ถูกนับว่าถูก คิดใหม่ตามพฤติกรรมจริง

| | เดินหน้า ตามป้าย | ตามพฤติกรรม | เลี้ยว ตามป้าย | ตามพฤติกรรม |
|---|---|---|---|---|
| ปรับร่วม + InfoNCE | 74% | **83%** | 32% | **23%** |
| สุ่ม | 33% | **42%** | 33% | **25%** |

**ข้อสรุปทั้งสองข้อไม่พลิก** เดินหน้า 83% เทียบสุ่ม 42% ยังเป็นสองเท่า และการเลี้ยว 23% เทียบ 25%
ยังไม่ผ่าน **และอาร์ม contrastive เป็นตัวที่ได้เพิ่มมากที่สุดฝั่งเดินหน้าและเสียมากที่สุดฝั่งเลี้ยว**
ซึ่งเป็นสิ่งที่แพลนเนอร์ที่อ่านเป้าหมายจริงควรทำ

**`--separability` มองไม่เห็นเพราะมันตรวจว่าแต่ละระดับมากกว่าระดับที่ต่ำกว่าเท่านั้น** ไม่มีอะไรตรวจว่า
ระดับต่ำสุดของตระกูลหนึ่งต้องต่างจากอีกตระกูล ทางแก้ที่ควรทำคือเก็บระดับใหม่ เช่น `wz0.60` แทนการ
เปลี่ยนป้าย เพราะได้สมดุลกลับมาและขยายช่วงการเลี้ยวของ B1 ให้เกินของแมลง ซึ่งตอนนี้กลับกันอยู่ (F105)

---

## 26. ทิศการเลี้ยวถูกจับให้ตรงกันทั้งสามชุด และลบทุกอย่างที่สร้างจากข้อมูลเก่า (2026-08-29)

**คำถามของผู้ใช้ว่า "โมเดลตัวไหนมาจากข้อมูลอะไร" พาไปเจอข้อบกพร่องสุดท้าย** ไล่โซ่ของเช็คพอยต์แล้ว
พบว่าตัวเดียวที่ยังไม่เคยตรวจคือชุด pretrain ของแมลง และมันเลี้ยวคนละทางกับชุดที่ใช้เป็นเป้าหมาย

```
c10f10t10  ใช้ pretrain               yaw บวก
c08f09t09  body ที่กันไว้  วงศ์เดียวกัน   yaw ลบ
B1                                    yaw ลบ  หลัง F106
```

**F84 มีข้อมูลนี้อยู่แล้วแต่ปล่อยช่องตารางว่าง** — มันจับการกลับทิศของการเดินข้างได้และรายงานเป็นหัวข้อ
ส่วนการเลี้ยวกลับทิศแบบเดียวกันแต่ไม่มีใครเห็น เพราะคอลัมน์เทียบไม่ได้ถูกกรอก

**ตัววัดสามตัวที่ลองแล้วเชื่อไม่ได้** ผลต่างควอเทอร์เนียนหัวท้ายให้เครื่องหมายเดียวกันทั้งสอง body และ
มุมเส้นทางแบบ unwrap ให้บันไดที่กลับหัว **ทั้งคู่ปนเปื้อนจากการที่ลำตัวแมลงส่ายทุกก้าว** ซึ่งเป็นเหตุผล
เดียวกับที่ F63 ปฏิเสธ Euler angles ตั้งแต่แรก **ตัวที่ตอบได้คือวาดเส้นทางแล้วดู**

**และผู้ใช้ชี้ว่าต้องบอกทิศในกรอบภาพได้** เพราะกล้องสองฉากเหมือนกัน ทิศบนจอจึงเป็นภาษาที่ใช้ร่วมกัน
ได้ — และถ้าบอกไม่ได้ ก็เท่ากับยอมรับว่าภาพไม่พาความหมายข้ามร่าง ซึ่งเป็นข้ออ้างของโครงการเอง
วัดจากแกนหลักของเงาหุ่นในภาพ ทั้งสามชุดตอนนี้หมุนทวนเข็มนาฬิกาบนจอเหมือนกัน

**การแก้ยึด c10f10t10 เป็นหลักตามที่ผู้ใช้สั่ง** เพราะ pretrain แพงที่สุด ส่วนการเก็บข้อมูลกับ
fine-tune ทำใหม่ได้ เพิ่ม `--spin_sign` ให้ collector และ `--turn_sign` ให้ตัวตรวจ แล้วเก็บใหม่
16 คลิปของ c08f09t09 และ 16 คลิปของ B1

| | ระดับ 1 | 2 | 3 | 4 |
|---|---|---|---|---|
| `beh12_c10f10t10_flat` pretrain | +0.0032 | +0.0141 | +0.0363 | +0.0775 |
| `beh12_c08f09t09_flat` held out | +0.0069 | +0.0215 | +0.0407 | +0.0863 |
| `beh12_b1_flat` คลัง | +0.0105 | +0.0268 | +0.0401 | +0.0807 |

**ครั้งแรกที่ทั้งสามชุดตรงกัน** และมันปลดล็อกการทดลอง stage 2 บนสองหุ่นพร้อม yaw ในเป้าหมายร่วม
ซึ่งก่อนหน้านี้จะสอนหัวที่แชร์กันด้วยเครื่องหมายที่ขัดกันเอง

**แล้วลบทุกอย่างที่สร้างจากคลิปเลี้ยวชุดเก่า** — 32 คลิปถูกเขียนทับที่พาธเดิม เพราะฉะนั้นแคช embedding
ที่ผูกกับพาธจะจ่ายภาพเก่าให้โดยไม่มีใครรู้ ซึ่งเป็นกับดักที่เงียบที่สุดในบรรดาที่เจอมา

```
แคชทั้งหมด                    18 G   สร้างใหม่ได้  และอันตรายถ้าเก็บไว้
เช็คพอยต์ฝั่ง B1 ทั้งหมด          3.8 G  ทั้ง _v2 และของก่อนหน้า
ผลลูปทั้งหมด                          ทั้ง B1 และแมลง  เพราะคลิปเลี้ยวเปลี่ยน
projector.pt กับ projector_c08f09t09  ฟิตบนคลิปที่เปลี่ยนไปแล้ว
คืนพื้นที่                       21 G
```
...
- **ต่อ Froude/embodiment เข้า `wm/data/embodiment.py`** — เพิ่ม `GECKO_DT`, `_gecko()`, entry ใน
  ...
  เจ็บมาแล้วกับ F62/F108): ขับ CPG จริง บันทึก quaternion จริง เทียบทิศทางที่วัดได้จริง — ได้ว่า
...
## 28. wm.train ไม่ใช่ adaptation — เจอ pipeline ที่ถูกต้องจริง แล้วปิด B1 ทั้ง ground/transfer/control (2026-09-09)

**จุดเริ่ม: fine-tune ด้วย `wm.train --init_ckpt` ทำให้ B1 แย่ลงกว่าไม่ทำอะไรเลย (F182)** — B1
median rho ตกจาก 0.264 (zero-shot) เหลือ 0.231 หลัง fine-tune แก้ confound เรื่อง `body_stats`
recompute เงียบๆ ตอนเพิ่ม B1 เข้าไปแล้ว (F182, เพิ่ม flag `--body_stats_from`) ก็ยังไม่ดีขึ้น (0.234)
— channel forward ยังคง negative ทั้งสองแบบ ชี้ว่าปัญหาลึกกว่าการตั้งค่าสเกล

**คำถามจากผู้ใช้ที่เปลี่ยนทิศทั้งหมด: "เราทำ fine-tune ตาม LAC-WM จริงหรือเปล่า"** เช็คแล้วพบว่า
**ไม่ใช่เลย** — `wm.train --init_ckpt` retrain ITM+FTM+decoder ทั้งหมด+body_head+probe รวมกันภายใต้
multi-task loss เต็มรูปแบบ ไม่ใช่ขั้นตอน adaptation ที่ LAC-WM กำหนด (fine-tune ITM/FTM อย่างเดียว →
fit projector ใหม่ → (ถ้าจำเป็น) fine-tune ร่วมกัน → refit body_head) ซึ่งสคริปต์ทั้งหมดมีอยู่แล้วใน
`wm/adapt.py`, `wm/fit_projector.py`, `wm/adapt3.py`, `wm/fit_body_head.py` แค่ไม่เคยถูกร้อยเข้าด้วยกัน
เป็น pipeline เดียว และไม่เคยมี closed loop ที่ใช้ body_head จริงเลยในประวัติศาสตร์โปรเจกต์ (เจอ
ข้อความเก่าที่เขียนไว้ตรงๆ ว่า "การทดลองนี้ยังไม่เคยสร้าง")

**ลบของเก่าทั้งหมดที่พิสูจน์แล้วว่าผิดกลไก** — checkpoint 4 ตัว (~1.5G) + cache 2 ไฟล์ (~8.6G) จาก
`wm.train`-based fine-tune, revert `--body_stats_from` flag ออกจาก `wm/train.py`, ย่อ F182 ใน
...
- **F183**: median rho ของ B1 กระโดดจาก 0.264 (zero-shot) เป็น **0.572** — ทุก channel ดีขึ้น รวมถึง
  forward ที่ไม่เคยเป็นบวกจริงในทุก checkpoint จาก `wm.train` เลย
- **F184**: ทดสอบ transfer goal ข้าม embodiment จริง (`score_by_body_motion.py`) — mode A (ไม่มี
  rollout) ผ่าน chance ชัดเจน (38-46% เทียบ 28%) ตรงกับ F122 เก่า; mode C (goal มาจากวิดีโอ + rollout
  ผ่าน FTM) ไม่ผ่านเลยทุก horizon 1-10 — ตรงกับ pattern "rollout ไม่ช่วย" ที่เจอซ้ำมาตลอดโปรเจกต์
- **F185**: สร้าง closed loop ที่ render จริง ใช้ body_head ครั้งแรกในประวัติศาสตร์โปรเจกต์ (ของเดิม
  ทุกอันใช้ raw embedding distance ที่รู้แล้วว่าผิดตั้งแต่ F111-F115) — direct ชนะ rollout ชัดเจนอีกครั้ง
  (median speed error 15.6% เทียบ 43.7%, ดีกว่า baseline เก่า F85/F91 ด้วยซ้ำ) เจอบั๊กกล้อง allocentric
  ที่ป้อนเข้า rollout ผิด (แก้แล้ว, ผลไม่เปลี่ยนทิศทาง — ยืนยันว่า rollout พังจริงไม่ใช่เพราะกล้อง) และ
  วินิจฉัยจุดที่ direct เลือกผิดถึงต้นตอ: **B1 ไม่เคยมีคลิปไหนที่ yaw เป็น channel เด่นเลยสักคลิปเดียว
  (0.0% จาก 3,168 transitions)** เหมือนปัญหาเดียวกับที่ hexapod's own turn library ก็ไม่เคยมี yaw ชนะ
  forward เลย — พิสูจน์ด้วยตัวเลขจริงว่าเป็นปัญหาข้อมูลไม่สมดุล ไม่ใช่สถาปัตยกรรม

**ที่ยังไม่ได้ทำ (ตอนจบวันนั้น)**: ลอง refit `body_head` ด้วย `--latent projector` อย่างเดียว (ไม่ผสม
ITM) ตามบทเรียน F122 — ยังไม่ได้ทำจนถึงตอนนี้. gecko ยังรอผล CPG-amplitude sweep จาก session อื่น
→ **CPG fix ลงแล้วและ gecko ได้รันจริงในวันถัดมา ดูหัวข้อ 29**


---

## 29. Gecko ผ่าน pipeline ที่ถูกต้องแล้วพังที่ stage 4 และการไล่หาสาเหตุที่ล้มสมมติฐานตัวเอง (2026-09-10)

**สรุปหนึ่งบรรทัด**: เอา pipeline เดียวกับที่กู้ B1 สำเร็จ (F183) มารันกับ gecko แบบไม่แก้อะไรเลย — พังที่
stage 4 (ratio 1.010 = เรียนแค่ค่าเฉลี่ย) แล้วไล่หาสาเหตุ 3 ชั้น จนได้ข้อสรุปที่**ค้านสมมติฐานที่ตั้งเอง**

> ตัวเลขและการอ่านผลทั้งหมดอยู่ใน **F189** (`FINDINGS.md`); คำถามที่ยังเปิดค้างอยู่คือ **Q20**
> (`OPEN_QUESTION.md`) — เลือกระหว่าง budget / yaw telemetry / physics noise เป็นตัวถัดไป

**แก้บั๊ก dt ก่อน** — `close_loop_direct_froude.py` hardcode `dt=0.05` ทุก embodiment ทั้งที่ B1/gecko
ใช้ 0.02 (แก้เป็น lookup ตาม `--embodiment`). ตรวจแล้วผลเก่าไม่กระทบ: field นี้ถูกใช้แค่ตอน save
metadata ไม่มีสคริปต์ไหนอ่านกลับไปคำนวณเลย — แต่ถ้าไม่แก้ ตอนรัน gecko จะผิด 2.5 เท่าแบบเงียบๆ

**รัน gecko ผ่าน `wm.finetune_new_body` (base เดียวกับ B1, budget เดียวกัน 9 clips/1000 steps)**:

| stage | B1 (F183) | gecko |
|---|---|---|
| 1 rollout ratio | 1.55 → **1.23-1.47** (ดีขึ้น) | 1.57 → **1.62-1.63** (แย่ลง) |
| 2 projector rollout-gap | 0.393 | **0.230** (ดีกว่า B1) |
| 4 held-out ratio | 1.330 → **0.751** | 1.837 → **1.010** (= ค่าเฉลี่ย) |

stage 2 fit ดีกว่า B1 แต่ stage 4 ลงตรงค่าเฉลี่ยพอดี — `a → z` เรียนได้ แต่ `z` ที่ได้อ่านเป็น Froude ไม่ได้

**ลอง stage 3 → แย่ลง (1.010 → 1.146)** ซึ่งเป็นหลักฐาน ไม่ใช่ความล้มเหลว: `wm.adapt3` มีไว้แก้เคส
"stage 2 fit ไม่ไหว" พอดี ถ้ามีโครงสร้างจริงซ่อนอยู่ นี่คือ lever ที่จะเจอ — ผลคือ `/hold` ratio ไต่ขึ้น
0.620 → 0.786 **ระหว่าง** train ขณะ train loss ลง (ลายเซ็น overfit บน 9 clips) และ `cond`/`family`
selection ค้างที่ 0%/0% ตลอด 3000 steps. เพิ่ม capacity แล้วไม่เจออะไร เพราะไม่มีอะไรให้เจอ

**แยกดูรายช่อง (per-channel) → ล้มสมมติฐาน "yaw พังช่องเดียว"**:

| latent | forward | lateral | yaw | median |
|---|---|---|---|---|
| itm (ฝั่ง goal, เฟรมจริง) | +0.190 | +0.150 | +0.171 | +0.171 |
| proj (ฝั่ง candidate, ที่ planner ใช้จริง) | +0.045 | −0.011 | +0.032 | **+0.032 (noise)** |

ทั้งสามช่องอ่อนพอกันหมด — yaw ไม่ได้แย่กว่า forward เลย ดังนั้นปัญหา yaw telemetry ที่รู้อยู่แล้วไม่ใช่
ตัวที่ทำให้ stage 4 พัง. และ**สัญญาณที่มีจริงใน ITM path หายไประหว่างทางไป projector path** (0.171 →
0.032) ซึ่งบน B1 ไม่หาย (0.517 → 0.572)

**สมมติฐาน "babble แคบเกิน" — วัดจริง แล้วทดสอบจริง**: gecko babble แคบกว่า B1 ~2 เท่าทุกแกน (action
std 0.225 vs 0.435, range 1.23 vs 1.80, step-to-step 0.121 vs 0.225). ลองสองวิธี:

1. **jitter ในคลิป (decaying-prior แบบ APEX: CPG params random-walk + noise ramp) — ตกการทดสอบของ
   ตัวเอง และนี่คือประเด็น**: ดัน action std ไปแตะ B1 เป๊ะ (0.225 → 0.453) แต่ความครอบคลุมความเร็วจริง
   **แย่ลงเป็นเส้นตรง** (0.031 → 0.026 → 0.022 → 0.021). PID low-pass jitter ทิ้ง gait เลยเสื่อมเป็น
   ย่ำอยู่กับที่ แทนที่จะเดินได้หลายความเร็ว — **entropy ใน command space ไม่ใช่ coverage ของ dynamics**
2. **ต้นเหตุจริงของความแคบคือ sweep อยู่ในโซนอิ่มตัว ไม่ใช่ noise น้อย**: วัด freq sweep ได้ froude_fwd
   2.0 → 0.033, 3.0 → 0.090, 4.0 → 0.128, 5.0 → 0.133, 6.0 → 0.121 คือ**อิ่มตัวเหนือ ~4.5** และ babble
   เดิม sweep `--freq 3.5/4.5` ซึ่งอยู่ใน**ที่ราบด้านบนทั้งหมด** ทุกคลิปเลยเดินความเร็วเดียวกันโดยโครงสร้าง

**เก็บ `data/gecko/babble_v3` ใหม่ 54 คลิป** — gait คงที่ต่อคลิปแต่ต่างกันข้ามคลิป: `freq` 2.0-6.0 ×
`lift` 0.40-0.80 (แกนที่ชุดเดิมตรึงไว้เป็นค่าคงที่ตลอด) × `turn`. ความครอบคลุมความเร็วดีขึ้นจริง:
spread ของความเร็วเฉลี่ยต่อคลิป 0.0191 → **0.0334** (+75%, จาก 33% เป็น 58% ของ B1)

**Gate ที่ pre-register ไว้คือ stage 1 — และไม่ผ่าน**: budget เดียวกัน (9 clips/1000 steps) ผลคือ
1.57 → 1.59 / 1.59 → 1.59 / 1.53 → 1.53 / 1.42 → 1.39 คือ**จาก "แย่ลงชัดๆ" กลายเป็น "แบนราบ"** —
ขยับถูกทาง แต่ไม่ใกล้การดีขึ้นชัดเจนแบบ B1 เลย. อ่านตาม pre-registration: ปิด gap ได้แค่ครึ่งทาง (58%)
และ gate ยังไม่ผ่าน → **ความแคบของ babble ไม่ใช่สาเหตุทั้งหมด** เหลือคำอธิบายที่ pre-registration
ระบุไว้เอง คือ dynamics ของ gecko เองมี noise สูงเกินกว่า budget นี้จะเรียนได้ (สอดคล้องกับที่วัดไว้แล้ว:
yaw แกว่ง −62/+114/+124/+126 องศาใน 4 รันที่ config เหมือนกันเป๊ะ, quaternion ต่างกัน ~0.1% ระหว่าง
reload ฉากที่เหมือนกัน แม้ไม่สั่ง gait เลย)

**ที่ยังไม่ได้ทำ** เรียงตามน้ำหนักหลักฐาน: (ก) เพิ่ม budget การ adapt (9 clips/1000 steps ยืมมาจาก B1
ทั้งที่ gecko เป็นหุ่นที่ noise สูงกว่า — budget เท่ากันไม่ได้แปลว่าความยากเท่ากัน), (ข) แก้ yaw telemetry
(`np.unwrap` ambiguity — ไม่ใช่สาเหตุของ stage 4 ตามตารางรายช่อง แต่มันใส่ variance ปลอมเข้าไปในช่องที่
`body_head` ต้อง fit: gecko yaw std 0.51 เทียบ B1 0.027), (ค) ลด noise ของ physics เอง

**ที่ยืนยันแล้วว่าไม่ใช่ปัญหา**: กลไก (รัน B1 ผ่าน code path เดียวกันได้ผลดี), stage 3, และการ "เก็บ
babble ให้กว้างขึ้น" เวอร์ชันที่ไล่ตามสถิติ action (ทำแล้วแย่ลงจริง)

---

## 30. เจอบั๊กการวัดสองตัวใน gecko — แกน body frame ตั้งฉากขึ้นฟ้า และ polarity ที่อ่านกลับด้าน (2026-09-10, ต่อจากหัวข้อ 29)

**สรุปหนึ่งบรรทัด**: ข้อสรุปครึ่งหนึ่งของหัวข้อ 29 **ผิด** เพราะวัดผ่านบั๊กสองตัว — พอแก้แล้วภาพเปลี่ยนหมด
รายละเอียดตัวเลขทั้งหมดอยู่ใน **F189** (เขียนทับของเดิมทั้งเอนทรี)

**บั๊ก 1 — อ่าน stage-1 ratio กลับด้าน**: `rollout()` คืนค่า `hold_error / model_error` แปลว่า **ยิ่งสูงยิ่งดี**
วัดดิบๆ: B1 stage 1 ทำให้โมเดล**แย่ลง** (h=1 error 3.627 → 3.698) แต่ B1 **สำเร็จ** ที่ stage 4;
gecko stage 1 ทำให้**ดีขึ้น** (3.994 → 3.878) แต่ gecko **ล้มเหลว** → **stage-1 ratio ไม่ทำนายผลปลายทางเลย
ใช้เป็น gate ไม่ได้** (gate ที่ pre-register กันไว้เมื่อวานจึงวัดผิดสิ่ง)

**บั๊ก 2 — แกน forward ของ gecko ชี้ขึ้นฟ้า**: `forward_axis`/`heading` ใช้แกน body x ซึ่งเงาบนระนาบแนวนอน
ยาวเฉลี่ยแค่ **0.065** (ต่ำสุด 0.000) → **99.9% ของเฟรมอยู่ในโซนที่ `arctan2` ไม่มีความหมาย** ผลคือ:

- heading กระโดดถึง **358 องศาต่อเฟรม** ที่ 50Hz (16.8% ของ step เกิน 90 องศา; B1 สูงสุด 2 องศา)
- `body_velocity` ฉาย velocity ลงบนแกนนี้ → **forward กับ lateral ก็เพี้ยนไปด้วย ไม่ใช่แค่ yaw**
- **ค่า Froude ที่เคยเชื่อว่า 0.133-0.138 "เท่ากับ hexapod/B1" ไม่เคยเป็นจริง**

แกนที่ถูกคือ **-(body y)** หาโดยวัดเทียบระยะเดินจริง ไม่ได้เดา: cos **+0.924** (เทียบของเดิม -0.916),
เงาแนวนอน **0.998** (เดิม 0.065), unstable **0%** (เดิม 99.9%), heading กระโดด **0 ครั้ง** (เดิม 16.8%)

**ผลของการแก้ (วัดด้วย ridge บน embedding แช่แข็ง ไม่มีโมเดลเทรนในลูป):**

| ช่อง | ก่อนแก้ | หลังแก้ | B1 |
|---|---|---|---|
| forward | +0.244 | +0.303 | +0.747 |
| lateral | +0.606 (พองเพราะเฟรมเพี้ยน) | +0.285 | +0.607 |
| yaw | **−0.042 (ตาย)** | **+0.362 ถึง +0.501** | +0.672 |

ช่อง yaw ที่เคยตัดทิ้งว่า "telemetry พังแก้ไม่ได้" จริงๆ เป็นบั๊กระบบพิกัด และตอนนี้แข็งแรงที่สุดในสามช่อง

**บั๊ก 3 — input ของ projector ไม่มีข้อมูลที่ถูกขอให้ทำนาย**: ridge บน ground truth, action → body motion:
action **เฟรมเดียว** (สิ่งที่ `ActionProjector` ได้รับจริง) gecko +0.215 / B1 +0.459; แต่ **window 20 เฟรม**
gecko **+0.736** ≈ B1 +0.770 → พอให้ประวัติคำสั่งของตัวเอง 20 เฟรม gecko ก็ให้ข้อมูลพอๆ กับ B1
(ไม่ขัดกับ claim: เป็นคำสั่งมอเตอร์ของหุ่นเองที่ babble เก็บอยู่แล้ว ไม่ใช่ kinematics/expert/privileged)

**แก้ทั้งสองอย่างพร้อมกัน**: stage-4 held-out **1.016 → 0.970** (ต่ำกว่า 1.0 ครั้งแรก), projector-path rho
**0.032 → 0.249**

**ที่เหลือคือข้อจำกัดทางกายภาพ ไม่ใช่วิธีการ**: action ให้ 0.736 → **วิดีโอให้แค่ 0.374** → pipeline ได้ 0.249
(67% ของเพดานวิดีโอ) **วิดีโอคือคอขวด และเหตุผลคือ gecko เดินช้าจริงๆ** — Froude **0.038-0.048** เทียบ B1 0.126
และ**แบนราบตลอดช่วง freq 2-6 Hz** (คันโยกความเร็วที่เคยเชื่อว่ามี ไม่มีจริง)

**สองข้อสรุปของเมื่อวานตายที่นี่ ทั้งคู่เป็นของผมเอง**: (ก) "babble แคบไป 2 เท่า" วัดผ่านเฟรมที่พัง
(ข) `babble_v3` 54 คลิปที่เก็บมาเพื่อขยาย coverage สร้างบนคันโยกที่ไม่มีอยู่ — วัดใหม่แล้ว **v3 แย่กว่า**
(action ceiling +0.146 เทียบของเดิม +0.736) เพราะการตรึง gait ให้คงที่ต่อคลิป **ตัด noise ในคลิปทิ้ง ซึ่งเป็น
สิ่งที่ทำให้ mapping เรียนรู้ได้จริง** — ตรงข้ามกับเหตุผลที่ใช้เก็บ v3
