...
def paths_for(data_dir, episodes, prefix="middleloss"):
    out = []
    for ep in episodes:
        p = os.path.join(data_dir, f"{prefix}_ep{ep}.npz")
...
def all_paths(data_dir, prefix="middleloss"):
...
def main():
    ...
    train_paths = paths_for(data_dir, train_eps, args.prefix)
    if args.test_eps:
        test_paths = paths_for(test_dir, parse_list(args.test_eps), args.prefix)
    else:
        train_set = {os.path.basename(p) for p in train_paths}
        test_paths = [p for p in all_paths(test_dir, args.prefix)
