...
**0.0 is the no-learning line** the way 0.500 is for the binary leg probe.
  ...
  .venv/bin/python3 scripts/diagnostics/shared_body_target/body_motion_probe.py
  .venv/bin/python3 scripts/diagnostics/shared_body_target/body_motion_probe.py --ckpt wm/runs/stage2_clean/best.pt
"""
import argparse
import glob
import os
...
def gather(encoder, chunk, insect_dir, cache_path, itm=None, checkpoint=None):
    ...
    for name in ("insect", "b1"):
        if name == "insect":
            ...
            paths = sorted(glob.glob(f"{ROOT}/data/allocentric/fwd_b1_50hz/*.npz"))
...
def main():
    ...
    ap.add_argument("--insect_dir", default="data/allocentric/fwd_hex8body",
