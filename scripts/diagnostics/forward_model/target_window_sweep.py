...
real target will not do better.
  ...
  .venv/bin/python3 scripts/diagnostics/forward_model/target_window_sweep.py --device cuda
"""
import argparse
import os
import sys
...
from diagnostics.shared_body_target.body_motion_probe import DT, bands, insect_paths  # noqa: E402
...
def main():
    ...
    groups = {"insect": insect_paths(os.path.join(ROOT, args.insect_dir)),
              "b1": sorted(glob.glob(f"{ROOT}/data/allocentric/fwd_b1_50hz/*.npz"))}
