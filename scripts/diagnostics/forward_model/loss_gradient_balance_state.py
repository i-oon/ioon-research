"""F140 extended: adds `lambda_state`'s term, which F140 never measured -- its checkpoint
(`beh12_hex-b1_body3`) predates the flag. `beh12_state` (the checkpoint the whole RL arc trained
against) sets `lambda_state=1.5`, nominally above `lambda_recon`'s 1.0, but F140 already showed
...
Original F140 docstring, unchanged, for context on why this measures gradients and not losses:
