"""F149 extended: adds `lambda_state`'s term, which F149 never measured -- its checkpoint
(`beh12_hex-b1_body3`) predates the flag. `beh12_state` (the checkpoint the whole RL arc trained
against) sets `lambda_state=1.5`, nominally above `lambda_recon`'s 1.0, but F149 already showed
nominal weight is not gradient share -- the state term's target is the FTM's own predicted change
(pooled, offset-corrected) plus `z_proj(z)`, scored against the same standardised `body_motion` the
`body` head already targets, so its actual pull on `z` needs the same direct measurement, not an
inference from its lambda.

Original F149 docstring, unchanged, for context on why this measures gradients and not losses:
...
`recon` gradient would mostly be pulling `z` toward a solution it is not needed for.
  ...
  .venv/bin/python3 scripts/diagnostics/forward_model/loss_gradient_balance_state.py \
      --ckpt wm/runs/beh12_state/best.pt --embodiment b1 --dir data/egocentric/beh12_b1_ego_flat
"""
import argparse
import glob
import os
...
def main():
    ...
    state_model = None
    if cfg.lambda_state > 0 and "state" in ck:
        names = tuple(spec.split("=", 1)[0] for spec in cfg.sources) or ("default",)
        state_model = StateHead(cfg, cfg.body_dim, names,
                                use_delta=cfg.state_use_delta).to(device).eval()
    ...
    if state_model is not None:
        # matches wm/train.py:182,210 exactly: pred_next uses the REAL embodiment (not recon's
        # `None` above), delta = pred_next - e_t, state_target is the same standardised body_motion
        # the body head targets (batch["body_motion"] is reused for both in training)
        pred_next_state = ftm(e_t, z, args.embodiment)
        delta = pred_next_state - e_t
        terms["state"] = (cfg.lambda_state, F.mse_loss(state_model(delta, z, args.embodiment), b))
