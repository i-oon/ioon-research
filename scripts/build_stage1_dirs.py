...
def walks(path):
    with np.load(path) as data:
        head = data["head"]
    forward = head[-1, 0] - head[0, 0]
    lateral = abs(head[-1, 1] - head[0, 1])
    return forward >= WALK_FORWARD_M and lateral <= WALK_LATERAL_M
...
def evenly(items, keep):
    """`keep` items spread across the list, not its first `keep`.

    Episodes differ in what the expert was asked to do, so a prefix samples one end of that
    range and calls it the dataset.
    """
    if keep >= len(items):
        return list(items)
    index = np.linspace(0, len(items) - 1, keep).round().astype(int)
