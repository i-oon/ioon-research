...
# level, and turns running opposite to the insect's (F113-F115, F117). `data/allocentric/beh12_b1_flat` is the
...
[ -d data/allocentric/beh12_b1_flat ] || { echo "missing data/allocentric/beh12_b1_flat" >&2; exit 1; }
...
if [ -f "$ADAPTED" ]; then echo "skip $ADAPTED"; else
  PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True $PY -u -m wm.adapt \
    --ckpt $RUN/best.pt \
    --data data/allocentric/beh12_b1_flat --embodiment b1 --clips 9 --stratify --train_clips $CLIPS \
    --out "$ADAPTED"
fi
...
if [ -f "$PROJ" ]; then echo "skip $PROJ"; else
  PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True $PY -u -m wm.fit_projector \
    --ckpt "$ADAPTED" \
    --hex_dir data/allocentric/beh12_c10f10t10_flat --b1_dir data/allocentric/beh12_b1_flat \
    --exclude $HOLDOUT \
