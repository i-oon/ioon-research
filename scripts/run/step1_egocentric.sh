...
collect)
  ...
  $PY scripts/diagnostics/egocentric_view/check_appearance_leak.py --data "$HELD" --embodiment hexapod
...
measure)
  ...
  for SPEC in "hexapod $HELD nullsep_ego_hex" "b1 $TRAIN_B1 nullsep_ego_b1"; do
    set -- $SPEC
    $PY scripts/diagnostics/objective_experiments/null_separability.py --ckpt "$RUN/best.pt" \
