...
# $PY scripts/diagnostics/decoder/motion_decoder_ceiling.py --ckpt "$ALLO" --data "$ALLO_B1" \
...
$PY scripts/diagnostics/decoder/motion_decoder_ceiling.py --ckpt "$EGO" --data "$EGO_HEX" \
    --embodiment hexapod --bar -0.53 --reference 0.773 0.903 0.938 \
    --cache results/wm/cache/ego_hex.pt
$PY scripts/diagnostics/decoder/motion_decoder_ceiling.py --ckpt "$EGO" --data "$EGO_B1" \
