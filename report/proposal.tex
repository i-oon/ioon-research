...
ไฟล์ออกแบบ (CAD/URDF) โครงสร้างจลนศาสตร์ ตัวปรับเฉพาะหุ่นยนต์ หรือข้อมูลสาธิตของหุ่นยนต์เป้าหมาย งานวิจัยนี้จึงศึกษาว่า
พฤติกรรมการเคลื่อนที่ที่บันทึกจากหุ่นยนต์ตัวหนึ่งจะสามารถขับเคลื่อนหุ่นยนต์อีกตัวที่มีโครงสร้างต่างกันโดยสิ้นเชิงได้หรือไม่
โดยใช้เพียงวิดีโอจากมุมมองบุคคลที่หนึ่ง (egocentric) ผ่าน \textbf{พิกัดการเคลื่อนที่ของลำตัวร่วม} ซึ่งประกอบด้วยความเร็ว
ไปข้างหน้า ความเร็วด้านข้าง และอัตราการหมุน ปรับให้ไร้มิติด้วยเลขฟรูด (Froude number) โดยไม่ใช้แบบจำลองจลนศาสตร์
ไม่ใช้ไฟล์ URDF และไม่ใช้ข้อมูลสาธิตของหุ่นยนต์เป้าหมาย

\noindent หุ่นยนต์ที่ใช้คือแมลงกิ่งไม้จำลอง (\emph{Medauroidea extradentata}) ขนาด 18 องศาอิสระ ในโปรแกรม CoppeliaSim
...
\noindent ผลการดำเนินงานจนถึงปัจจุบันแสดงว่า พิกัดการเคลื่อนที่ของลำตัวร่วมสามารถถ่ายทอดข้ามร่างกายได้โดยไม่ต้องจับคู่เฟรม
หรือปรับเป้าหมายระหว่างหุ่นยนต์ทั้งสอง และพบว่ามุมมองของกล้องเป็นเงื่อนไขสำคัญที่ไม่เคยมีการตรวจสอบมาก่อน กล่าวคือ
มุมมองบุคคลที่สาม (allocentric) ทำให้ท่าทางของหุ่นยนต์บ่งบอกคำสั่งได้เอง เนื่องจากการเดินมีลักษณะเป็นคาบ แบบจำลองโลก
จึงทำนายอนาคตได้โดยแทบไม่ใช้ข้อมูลการกระทำ ขณะที่มุมมองบุคคลที่หนึ่งทำให้แบบจำลองกลับมาใช้การกระทำได้ ข้อจำกัดที่ยัง
เหลืออยู่คือแบบจำลองใช้การกระทำได้ในระดับหยาบเท่านั้น ยังไม่สามารถจัดอันดับความแตกต่างที่ละเอียดหรือทำนายต่อเนื่อง
ในระยะยาวได้
...
CAD or URDF description, a kinematic tree, a per-robot adapter fitted from that robot's own proprioception, or
demonstrations paired on the target. This research asks whether a behaviour recorded on one robot can drive a
\emph{structurally different} robot from egocentric video alone, through a \textbf{shared body-motion coordinate}
-- dimensionless forward, lateral, and yaw velocity, Froude-scaled so that it encodes behaviour rather than body
size -- with no kinematic model, no URDF, and no demonstrations of the target body.

The two bodies are a simulated 18-DOF stick insect (\emph{Medauroidea extradentata}) in CoppeliaSim and a 12-DOF
Unitree B1 quadruped in MuJoCo, whose action spaces share no dimension and no correspondence, so that no
proprioceptive representation could be shared between them without first being handed a joint mapping from
outside. A frozen V-JEPA2 encoder is combined with an Inverse Transition Model, a Forward Transition Model, and a
Motion Decoder, adapting the latent-action world-model architecture from manipulation to legged locomotion.

Results to date show that the shared body-motion coordinate transfers between the two bodies
\emph{correspondence-free}, with no frame pairing, alignment, or retargeting between them. They further show that
the camera viewpoint, treated as a setup detail in prior work, is a precondition rather than a convenience:
because locomotion is periodic, a third-person (allocentric) view lets the robot's own visible pose stand in for
the command, so the world model satisfies its prediction objective by completing the gait cycle and barely uses
the action at all, while an egocentric view restores measurable action-conditioning. The limitation currently
established is that this conditioning is coarse: the model uses the action at a single step but does not yet rank
fine differences between similar actions or roll out reliably over long horizons.
...
the thesis actually uses. Section 2.2 then organises the cross-embodiment literature on one axis --
\emph{what each method must be told about a robot before it can control it} -- because that is the
axis this thesis's claim lives on. Section 2.3 covers the one result that reduces that requirement to
nothing, for a single robot, and is therefore the closest precedent to this work. Section 2.4 states
what crossing between two \emph{different} robots additionally requires, and Section 2.5 shows why
the component that requirement depends on -- a world model that genuinely uses the action it is
given -- is not something the literature has established for locomotion. Section 2.6 states the
resulting gap and the claim of this thesis.
...
thing on all of them -- end-effector pose. Section 2.4 takes up why that premise does not survive the
...
whether a \emph{model} uses a quantity, presence-style metrics are insufficient on their own, for
reasons Section 2.5 develops.
...
than by architecture, the literature shows a clear pattern: the body description is always supplied
from somewhere, and the differences are about where from.
...
This is the cost the rest of the field is trying to avoid and the reference against which
sample-efficiency claims are measured.

\subsection{Design files: explicit morphology conditioning}
QWM (Danesh et al., 2026) conditions a world model on physical parameters -- limb lengths, masses,
torque limits -- read from each robot's CAD/USD description, and generalises zero-shot to unseen
quadruped morphologies (Figure~\ref{fig:qwm}). It is the strongest cross-morphology transfer result in
this area and the clearest contrast to this thesis: the morphology is \emph{supplied}, not inferred,
and its observation channel is proprioception. Notably, QWM routes what it calls ``unmodeled
real-world residuals'' through a separate dynamic latent -- an implicit concession that the design
file does not fully describe the physical body.
...
\subsection{Kinematic graphs and online system identification}
...
connects to which. A related family infers hidden dynamics parameters online from a window of recent
interaction, which removes the design file but requires access to the robot's internal signals and
enough new interaction to identify the parameters.

\subsection{Per-robot fitted adapters}
L3P (Zheng et al., 2025) wraps a shared latent policy backbone in a per-robot observation encoder and
per-robot latent-action decoder, so control transfers across diverse legged robots
(Figure~\ref{fig:l3p}). This shares \emph{structure} across bodies, and its decomposition is a close
structural parallel to this project's shared trunk with body-specific heads. The difference is the
channel and the direction of information: L3P fits its per-robot adapters from that robot's own
proprioception and foot force, with joint ordering and actuator conventions known.
...
Two vision-based cross-embodiment methods obtain the correspondence from data rather than from the
body. UniSkill (Kim et al., 2025) learns an embodiment-agnostic skill representation from video,
including human video, using an inverse/forward skill-dynamics pair (Figure~\ref{fig:uniskill}); its
invariance is \emph{optimised for} by training on cross-embodiment data with an alignment objective.
Demo-JEPA (He et al., 2026) reframes a demonstration as specifying a future \emph{goal state} rather
than actions to copy, and translates source demonstrations into target-compatible latent goals that
the target realises through its own world model. It requires neither a shared action space nor
action-level retargeting, but the translation is fitted from the target robot's own interaction
experience on trajectories paired with the source -- by retargeted end-effector replay in simulation,
and by Global Temporal Cycle Consistency alignment on real data.
...
\subsection{Latent actions in legged locomotion}
Closest in domain, Li et al.\ (2021) plan in a \emph{learned latent action space} for legged
locomotion and demonstrate it on both a hexapod and a quadruped. Their framework is re-instantiated
and trained \emph{separately per robot}: no latent space or dynamics model is shared or transferred
between them. It establishes that latent actions suit legged control, without making the latent space
itself the thing that transfers.

\subsection{Synthesis: what each method must be told}
Table~\ref{tab:positioning} places these methods on the axes that decide this thesis's claim. Reading
down the ``morphology information'' column, every method obtains the body description from inside the
body, from a design file, or from demonstrations paired on the target. Reading down ``disjoint action
spaces'', the methods that do transfer do so across bodies whose action space is shared by
construction, bridged by an adapter fitted with privileged access, or bridged by paired same-task
demonstrations. It is also worth noting that the vision-based latent-action methods are demonstrated
almost entirely on \textbf{manipulation}, where end-effector pose already provides a coordinate common
to every body in the corpus. Legged locomotion across different leg counts offers no such coordinate,
which Section 2.4 develops.
...
One result removes the requirement of Section 2.2 altogether, and it is the closest precedent to this
thesis. Hu, Chen, and Lipson (2025) show that a task-agnostic \emph{visual self-model} -- a forward
model mapping the current egocentric view and a motor action to the next egocentric view -- can be
learned for a legged robot from a single first-person camera and \textbf{random motor babbling}, with
no prior knowledge of the robot's morphology, kinematics, or task.
...
is a reasonable and sufficient basis for the choice. Section 2.5 argues that for a
\emph{cross-embodiment} world model it is not, because whether a video world model can use its action
at all turns out to depend on the viewpoint, and that question has not been asked in this literature.

% ============================================================================
\section{What Crossing Embodiments Additionally Requires}
Dropping the body description is necessary but not sufficient. Two further requirements follow from
the choice of bodies, and neither is satisfied by any method in Section 2.2.
...
The methods of Section 2.2 resolve this by being handed a mapping -- a kinematic tree, a fitted
adapter, or paired demonstrations. A method that is handed nothing must find a quantity that is
\emph{already} the same physical thing on both bodies and use it as the shared interface.
...
\subsection{The test pair: a stick insect and a quadruped}
The source body is a simulated stick insect (\emph{Medauroidea extradentata}), chosen because its
walking is exceptionally well characterised, a validated simulation model of the exact species exists
(Larsen et al., 2023), and as a slow, statically-oriented walker its gait is governed by leg
coordination rather than high-speed dynamics. Coordination in stick insects arises from decentralised
local rules between neighbouring legs (Cruse, 1990) rather than a central clock; each leg alternates a
\emph{stance} phase with a \emph{swing} phase, and the standard descriptors are the anterior and
posterior extreme positions bounding the stance stroke, the duty factor, and the inter-leg phase.
Insect gaits form a spectrum with speed: a tripod at high speed, and at low speed a wave-like pattern
in which legs are recruited back-to-front. The reference gait used here is a slow-walking pattern, and
both the recordings and the mature controller of Larsen et al.\ show a phase-staggered wave rather
than a clean tripod -- the real gait of a slow-walking stick insect, not a defect.

The target body is a Unitree B1 quadruped simulated in MuJoCo. The pair is deliberate: they share no
joint correspondence, no leg count, no gait family, and no simulator, so nothing but the body-level
coordinate of Section 2.4.1 can carry information between them.

% ============================================================================
\section{Do Video World Models Use the Action?}
The shared coordinate of Section 2.4 is only useful if a world model conditioned on it genuinely
depends on it. That turns out not to be safe to assume, and three recent results each see part of the
reason.
...
Together these establish that a video world model can appear action-sensitive by an offline metric
while its prediction does not actually depend on the action, and they locate the reason in three
different places (Table~\ref{tab:action-relevance}). What none of them asks is the question this
thesis is forced to confront: \textbf{how severe this becomes when the observation is periodic, and
whether the camera viewpoint determines it.} Locomotion is periodic by construction, and an external
view of a walking body shows a pose that is itself a function of gait phase. Whether that makes the
action redundant -- and whether an egocentric view removes the redundancy -- is measured in this
thesis (Chapter 3) rather than assumed, and it is the question Hu et al.\ (Section 2.3) never had to
ask because they never compared the two viewpoints.
...
established that this requirement can be removed entirely for a \emph{single} robot, using an
egocentric visual self-model, and that nobody has extended that to a second body. Section 2.4
established what such an extension additionally needs -- a shared coordinate that is not joint space,
Froude-scaled so it encodes behaviour rather than body size. Section 2.5 established that the
component this depends on, a world model that genuinely uses its action, is not something the
literature has demonstrated for locomotion, and that the influence of viewpoint on it is unexamined.
...
of UniSkill (Section 2.2.5). These are stated as directions, not commitments.
