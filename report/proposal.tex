...
\item The camera is \textbf{egocentric} (mounted on the body, first-person view), not the third-person /
  ...
  (Section 3.2.3) rather than an arbitrary setup detail: an allocentric camera lets the robot's own visible pose
...
\section{Stage 1: Prerequisite Substrate Check}
Before Stage 2 was attempted, five milestones were run on the Stage-1 leg-length substrate (Section 3.5.1) to
confirm the pipeline had something to work with: the morphology gap, encoder sanity, the two-sided latent
validation, transfer to a held-out leg length, and the effect of training-body diversity. Full results, read
against an explicit hypothesis for each, are reported as Experiment 1 in Chapter 4 (Section 4.1) rather than
here, so that this chapter states method and Chapter 4 states evidence without the two being interleaved. The
one-line summary needed to motivate what follows: the pipeline works and a trained latent beats the raw command
on a shared joint space, but this alone cannot show vision succeeds where proprioception structurally could not,
because all leg-geometry variants still share one 18-dimensional joint space (Section 1.1), which is
exactly the question Stage 2, on the disjoint-action-space pair, is designed to answer.
...
\noindent\textbf{Removing the candidate-scoring circularity.} Section 4.3 states why scoring among a curated
library presupposes a policy that can already produce the library's behaviours, which is circular for a genuinely
unseen robot. The proposed resolution is a trained policy (teacher-student) that needs only the ability to actuate the
robot, not a pre-existing behaviour set; this thesis has not yet demonstrated that such a policy, trained through
the current world model, out-performs the plain behaviour-cloned baseline of Section 4.3.4.
...
\section{Work Plan}
The Stage-1 substrate check and the full diagnostic chain of Chapter 4 are complete. What remains is the four
items above, ordered by how directly each bears on the thesis's central open question: whether the world model
can be made to rank actions precisely, not only condition on them coarsely.
...
Counterfactual-target training & Rebuild the forward-model objective against counterfactual rather than
  teacher-forced targets; re-run the ranking test of Section 4.3 & highest\\
Candidate-separation & Broaden the recorded behaviour library; scope a motor-babbling data-collection pass &
  high\\
Teacher-student policy & Train a policy through the current world model; compare against the behaviour-cloned
  baseline (Section 4.3.4) & high\\
...
single architectural change tried at this stage. \textbf{What this experiment does not do, and was never
designed to do, is reach a closed loop.} Every result above is an offline probe or an open-loop transfer score;
no controller was ever asked to drive a body toward a goal using this latent. That gap, proving the
representation is right without ever testing whether it can be \emph{used}, is exactly what motivated closing
the loop in Experiment 3, on the harder pair where the joint space no longer offers a shortcut.
...
\hline
1. Cross-morphology & Latent beats raw command on a shared joint space; three of four hypotheses confirm & Never
...
Chapter 5 discusses what this means for the thesis's contribution and why the counterfactual-target direction is
the most direct path past it.
...
converge on the same wall, a stronger and more specific result than an unexplained failure would be.
...
on the identical result. \textbf{The candidate-scoring circularity} (Section 4.3): the closed-loop test harness
depends on a curated library of the target robot's own recorded behaviours, which presupposes exactly the
capability, a controller for the target robot, that a genuinely unseen robot would not have. \textbf{The
viewpoint trade} (Section 4.3): the egocentric camera that is necessary for the world model to use the action at
all is the same camera under which plain behaviour cloning nearly fails (6\% of the reference distance, against
54\% allocentrically); this thesis does not resolve that tension, and reports both sides of it rather than the
one that is more favourable.

\section{Recommendations for Remaining Work}
Section 3.8 states these as a work plan; they are repeated here as the direct response to Experiment 3's wall.

\textbf{Counterfactual-target training} is the most direct next step, and the one properly-motivated fix this
thesis has not tested at true strength (Section 4.3.8): resuming the real simulator from the same state under a
...
\textbf{Breaking the candidate-scoring circularity} requires replacing scored candidates with a trained policy
that needs only the ability to actuate the target robot, not a pre-existing behaviour set (Section 2.4, following
Hu et al.'s use of unguided motor babbling for a single robot); this thesis has identified the resolution but not
yet demonstrated it out-performs the plain-cloning baseline of Section 4.3.4.
...
first suggested: coarse and family-level behaviour selection work well; exact fine-grained control does not yet,
and five independent interventions converge on that being a structural property of the current objective rather
than a component that merely needs fixing. Stating that limit precisely, rather than around it, is what makes the
next step, counterfactual-target training, tested at true strength, a specific, testable proposal rather than
an open-ended one.
