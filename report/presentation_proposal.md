...
|---|---|---|---|
| Preserved | Reduced | Improved | **Intended result** — a behaviour representation that maps to a shared body-motion coordinate. Note `z` itself keeps body identity, and measurably should: probe 0.974, and stripping it made transfer worse (F146, F21). |
...
- **Supervision is in zₜ-space → a shared body-motion coordinate.** We compare achieved-vs-target *behaviour*, not raw embeddings. **Not "body shape can't leak in"** — it does, by design: body identity is decodable from `z` at 0.974 (F146), and the per-body decoders need it. What is shared is the coordinate, not the latent.
